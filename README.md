...
## ⚠️ Important : mise à jour du 25/09 (correctif cache)

Une version précédente du jeu utilisait un "service worker" qui mettait la page en cache de façon trop agressive :
une fois qu'un navigateur avait vu le jeu une première fois, il ne voyait plus jamais les mises à jour, même
après avoir remplacé `index.html` sur GitHub. Ce mécanisme a été retiré.

**Cette fois, remplace les 2 fichiers `index.html` ET `sw.js`** (pas juste `index.html`) pour que le nettoyage
du cache s'applique correctement. Pour les prochaines mises à jour, remplacer `index.html` seul suffira à nouveau.
